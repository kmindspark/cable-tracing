...
def is_valid_successor(pt, next_pt, depth_img, color_img, pts, pts_explored_set, cur_dir, black_pixel_distances, lax=False):
    ...
    no_black_on_path = has_black_on_path(color_img, pt, next_pt) <= (0.4 if not lax else 0.6)
...
def is_similar(pt, next_pt_1, next_pt_2):
    ...
    return cos_angle > COS_THRESH_SIMILAR #or (np.linalg.norm(pt - next_pt_1) - np.linalg.norm(pt - next_pt_2)) < 1
...
def dedup_candidates(pt, candidates, depth_img, color_img, pts, pts_explored_set, cur_dir, black_pixel_distances):
    ...
    for lax in [False, True]:
        ...
        for tier in range(len(candidates)):
            if tier > 1 and len(filtered_candidates) > 0:
                return filtered_candidates
...
if __name__ == "__main__":
    img_path = 'data_bank/series_simple/1640295900/color_0.npy' #'data_bank/large_figure8_simple/1640297369/color_0.npy'
    ...
    start_point_1 = np.array([58, 156]) 
    start_point_2 = np.array([57, 162]) 
