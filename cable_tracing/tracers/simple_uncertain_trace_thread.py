...
def is_valid_successor(
    pt, next_pt, depth_img, color_img, pts, pts_explored_set, cur_dir, image_height, image_width, lenient=False
):
    # if lenient:
    #     print("lenient cand:")
    ...
    if not 0 <= next_pt_int[0] < image_height:
        return False
    if not 0 <= next_pt_int[1] < image_width:
        return False
    ...
    if (
        next_pt_int[0] < 0
        or next_pt_int[1] < 0
        or next_pt_int[0] >= color_img.shape[0]
...
def is_similar(pt, next_pt_1, next_pt_2):
    ...
    return (
        cos_angle > COS_THRESH_SIMILAR
        and (np.linalg.norm(pt - next_pt_1) - np.linalg.norm(pt - next_pt_2)) < 1
...
def dedup_candidates(
    ...
    image_height,
    image_width,
    num_pts_to_consider_before_ret=NUM_POINTS_TO_CONSIDER_BEFORE_RET,
):
    ...
    for lenient in (
        [False, True] if num_pts_to_consider_before_ret is not None else [False]
    ):  # TODO: add True in first list if we want to consider lenient
        # if lenient:
        ...
        for tier in range(len(candidates)):
            ...
            for i in range(len(cur_candidates)):
                ...
                counter += 1
                if (
                    num_pts_to_consider_before_ret is not None
                    and len(filtered_candidates) >= num_pts_to_consider_before_ret / counter
                ):
                    return filtered_candidates
        # if lenient:
        #     plt.show()
...
def step_path(image, start_point, points_explored, points_explored_set, image_height, image_width, start_dir=None):
    ...
    cur_dir = (
        normalize(start_point - points_explored[-1]) if len(points_explored) >= NUM_POINTS_BEFORE_DIR else start_dir
    )
    ...
    deduplicated_candidates = dedup_candidates(
        ...
        image_height,
        image_width,
        num_points_to_consider_before_ret,
    )
        ...
        if np.linalg.norm(lns_indx[-1] - lns_indx_new[-1]) < 3:  # 2.5:
            if np.max(np.linalg.norm(lns_indx - lns_indx_new, axis=-1)) < 15:  # 4.5:
                ...
                if abs(len(path) - len(new_path)) < 2:
                    ...
                    if (
                        np.linalg.norm(np.array(path[: min_len - 1]) - np.array(new_path[: min_len - 1]), axis=-1).sum()
                        == 0
                    ):
                        continue
                return True
...
def trace(
    image, start_point_1, start_dir=None, timeout=30, bboxes=[], viz=True, exact_path_len=None, viz_iter=0, endpoints=[]
):
    ...
    while len(active_paths) > 0:
        ...
        if (
            len(endpoints) > 0
            and np.min(np.linalg.norm(endpoints - active_paths[0][0][-1][None, :], axis=-1)) < 20
            and len(active_paths[0][0]) > 15
        ):
            finished_path, finished_set_path = active_paths.pop(0)
            finished_paths.append(finished_path)
            finished_set_paths.append(finished_set_path)
        ...
        if len(step_path_res) == 0:
            ...
            for new_point_idx, new_point in enumerate(reversed(step_path_res)):
                ...
                if keep_path:
                    ...
                    new_set = get_updated_traversed_set(
                        image,
                        cur_active_path[1],
                        cur_active_path[0][-1],
                        new_point,
                        new_point_idx < len(step_path_res) - 1,
                    )
                    get_updated_traversed_set_time += time.time()
    ...
    if viz and len(finished_paths) > 0:
        ...
        fig.suptitle(
            f"All {len(finished_paths)} valid paths traced by cable until first knot in {side_len} x {side_len_2} grid."
        )
